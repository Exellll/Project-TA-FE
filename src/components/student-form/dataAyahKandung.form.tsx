...
const DataAyahKandung: React.FC<StudentProps> = ({
  step,
  setStep,
  stepStatic,
  setStepStatic,
  ...
  const { handleCreate, handleUpdate, errors, register, control, isLoading, reset } = useParentsForm(
    "ayah",
    setStep,
    setStepStatic,
    userId!
  ...
  useEffect(() => {
    if (data && Array.isArray(data)) {
      const parent = data.find(
        (item) => item.gender === "male" && item.type === "parent"
      );
  ...
  return (
    <Timeline.Item
      connect="both"
      endClassName={stepStatic < 2 && !userId ? "bg-gray-400" : "bg-[#1362fc]"}
      startClassName={stepStatic < 1 && !userId ? "bg-gray-400" : "bg-[#1362fc]"}
    >
      <Timeline.End box={false}>
        <button
          onClick={() =>
            handleStep(2) && setStep((prevStep) => (prevStep === 2 ? 0 : 2))
          }
...
export default DataAyahKandung;
