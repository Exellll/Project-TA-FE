...
const DataWali: React.FC<StudentProps> = ({
  step,
  setStep,
  stepStatic,
  setStepStatic,
  ...
  useEffect(() => {
    if (data && Array.isArray(data)) {
      const parent = data.find(
        (item) => item.gender === "male" && item.type === "wali"
      );
  ...
  return (
    <Timeline.Item
      connect="both"
      endClassName={stepStatic < 4 && !userId ? "bg-gray-400" : "bg-[#1362fc]"}
      startClassName={stepStatic < 3 && !userId ? "bg-gray-400" : "bg-[#1362fc]"}
    >
      <Timeline.Middle
        className={
          stepStatic === 3 && !userId
      ...
      <Timeline.End box={false}>
        <button
          onClick={() =>
            handleStep(4) && setStep((prevStep) => (prevStep === 4 ? 0 : 4))
          }
