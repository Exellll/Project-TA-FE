...
export default function UploadPaymentModal({
    ...
    const { handleCreate, handleUpdate, refetch, setValue, reset } = useStudentTransactionForm({
        page: 1,
        limit: 10,
        search: "",
    });
    return (
        <Modal open={isOpen} backdrop={false} className="flex flex-col bg-white w-full max-w-md rounded-xl">
            <form onSubmit={async (e?: React.BaseSyntheticEvent) => { await handleUpdate(e); refetch(); }}>
